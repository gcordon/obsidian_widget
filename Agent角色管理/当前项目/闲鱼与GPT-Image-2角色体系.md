...
| 发布、改价、删除、下架或其他账号副作用 | 用户批准门；用户手动执行 |
...
| 发布、改价、删除或下架 | 不启动发布执行 Agent | 协调 Agent 准备操作包 → 用户确认并手动执行 |

### 5. 当前数据下的实际角色分配

当前最新快照（2026-07-27 13:59:17）为：曝光 `43`、浏览 `9`、想要 `0`；本次未记录咨询和成交。

本轮只应激活：

- 项目协调 Agent：定义下一次观察时间和需要记录的指标。
- 闲鱼市场情报 Agent：针对下一个实验调查同类商品标题、首图和价格样本，区分平台规则与竞品做法。
- 闲鱼运营与数据 Agent：把市场情报与项目数据合并，只提出一个单变量实验。
- 质量与实验评估 Agent：在执行前检查数据起点、标题字数、单变量和观察条件。

暂不激活：

- 视觉创作 Agent：尚无足够证据证明首图或视觉需要修改。

当前已确认首轮只调整标题，因此不启动视觉创作 Agent。评估 Agent 在执行前检查新标题不超过 20 字且实验保持单变量；用户手动修改后建立新数据起点。若后续实验涉及图片，再进入“方案汇总 → 用户回复开始生图 → 视觉生产 → 质量评估 → 用户批准”的链路。
