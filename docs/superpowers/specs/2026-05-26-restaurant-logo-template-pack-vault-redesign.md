...
本次允许彻底改造当前项目：可以重写、合并、重命名、移动、归档或删除不再服务目标的笔记。判断标准只有一个：是否更有利于长期学习 AI 餐饮 Logo，并把学习成果沉淀为模板包、Prompt 包、风格包和素材包。

实施时需要遵守三条约束：

1. 不破坏可追溯性：重要原始资料、Demo 和历史说明可以被降级或归档，但不能在没有替代入口和说明的情况下直接消失。
2. 不为了复杂而复杂：可以使用 Obsidian Bases、Canvas 或新目录，但必须服务学习、总结和资产沉淀。
3. 不把重点放在接单流程：长期方向是模板包、Prompt 包、风格包和素材包，接单只作为未来可能的变现分支。
...
本次可以改造整个 vault，包括当前所有 Markdown、索引页、模板、系统说明和目录结构。优先处理这些核心文件：
...
默认不优先修改这些内容，但如果它们阻碍新的学习和资产沉淀结构，也可以移动、归档或重写说明：
...
其中 `.obsidian/` 和 `.agents/skills/` 仍应谨慎处理：除非明确需要优化 Obsidian 配置或 skills 协作方式，否则不改内部实现。
...
已有 Prompt 文件先按使用价值评估，再决定保留、合并、重写、归档或删除；最终以餐饮 Logo 学习和资产包沉淀为准。
...
本轮不要求完成真实销售页，但可以重写作品集结构，让它为后续销售页、模板包说明和资产打包做准备。
...
本轮优先建立统一元数据规则。可以先补核心页面，也可以在实施时对全库做系统性补齐，取决于实际改造范围和维护成本。
...
- `obsidian-bases`：不强制使用；如果它能明显提升案例、Prompt、资产候选的查看效率，可以纳入本轮改造。
...
6. 周复盘能总结学习进步和商业资产进度。
7. 重要原始资料、Demo 和图片有清楚去向：保留、归档、替代或删除原因明确。
8. 不引入与学习和资产沉淀无关的复杂维护负担。

## 风险与处理

### 风险：改造过度，学习压力更大

处理：允许系统性重写，但分批推进；优先完成学习闭环和资产沉淀闭环，再处理低价值旧内容。
...
处理：允许重命名、移动、归档或删除，但实施前先列出映射关系；涉及图片和 Demo 时同步更新引用或保留替代入口。

### 风险：未来需要 Bases 视图但当前元数据不完整

处理：先定义统一元数据；如果实施中要创建 `.base` 视图，就同步补齐它依赖的核心字段。
